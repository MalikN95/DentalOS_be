...
- **WhatsApp** (`senders/whatsapp.sender.ts`) — Meta WhatsApp Cloud API, plain HTTPS `fetch`. Needs `WHATSAPP_ACCESS_TOKEN` + `WHATSAPP_PHONE_NUMBER_ID` (see `.env.example`); no-ops with a logged warning until set. Free-form text (`type: "text"`) only delivers inside the 24h customer-service window; `message.whatsappTemplate` sends an approved `type: "template"` instead for proactive sends outside that window — see appointment reminders below.
...
**Inbound WhatsApp** (`modules/whatsapp/`) is the receiving side of the same integration — Meta calls this, the sender above never does:
- `GET /api/whatsapp/webhook` — one-time handshake Meta runs when you save the webhook URL in the App dashboard. Echoes back `hub.challenge` if `hub.verify_token` matches `WHATSAPP_VERIFY_TOKEN` (any string you pick), else `401`.
- `POST /api/whatsapp/webhook` — delivery of actual messages. Verifies the `X-Hub-Signature-256` header (HMAC-SHA256 over the raw body, keyed with `WHATSAPP_APP_SECRET`) if that env var is set, else logs a warning and skips verification (fine for local dev). Text messages are matched to a `PatientEntity` by phone (`ChatService#receiveWhatsAppMessage`, digits-only comparison) and stored as an inbound `PatientMessageEntity` — same timeline the kabinet's chat UI already reads for outbound sends. Unknown numbers are logged and dropped. Both endpoints are `@Public()` (Meta calls them unauthenticated) and always return `200` — Meta retries the same payload on non-2xx.
- Single caveat: `WHATSAPP_PHONE_NUMBER_ID` is one global number for the whole deployment, not per-clinic — if two clinics on the same deployment have a patient with the same phone number, the match is ambiguous (picks the most recently created patient).

**WhatsApp appointment reminder templates** (`modules/reminders/appointment-reminder-templates.ts`): `ReminderProcessorService#resolveWhatsAppTemplate` picks a Meta-approved template by the reminder's offset from the appointment (`120` minutes → `appointment_notification_hours_ru`, `1440` → `appointment_notificatin_dey`; both take `{{1}}` = patient first name, `{{2}}` = appointment time as `HH:mm`). An offset with no matching entry falls back to free-form text as before. Two things to verify in Meta before this works: the template must show **Активный** (not "На проверке" — Meta review can take up to 24h), and `languageCode` in that file must match what the template's detail page shows, not the language of its body text.
...
A patient-facing counterpart to the staff kabinet — login by phone (WhatsApp magic link), view/cancel own appointments, read and reply to messages.

- **Login**: `SmsAuthController`/`SmsAuthService` (`src/modules/auth/sms-auth.*`), registered in `AuthModule`, routed as `POST /api/auth/:clinicSlug/sms/request` (mirrors `BookingController`'s `booking/:clinicSlug` pattern) and `POST /api/auth/:clinicSlug/sms/verify`. No code entry: `requestLoginLink` generates a random 32-byte token (`base64url`), stores its SHA-256 hex digest in `OtpCodeEntity.codeHash`, and sends a WhatsApp link (`{CORS_ORIGIN}/portal/:slug/magic?token=...`, copy in `notification-copy.ts#magicLinkLoginCopy`) via `NotificationChannel.WHATSAPP`. The frontend's `/portal/:slug/magic` page shows one button; clicking it posts the token straight to `verify` (`MagicLinkVerifyDto`, no `:clinicSlug` needed — the token alone identifies clinic + phone via `SmsAuthService#verifyLoginLink`'s direct hash lookup), which creates/links a `UserEntity` (`role: patient`) to the matching `PatientEntity` exactly like the old OTP flow. Deliberately never auto-verifies on page load — WhatsApp (and most chat apps) prefetch shared links to build a preview, which would burn a single-use token before the patient ever taps it; the verify call only fires from an explicit client click.
...
- Link TTL is 15 minutes (`LINK_TTL_MS` in `sms-auth.service.ts`) — longer than the old 4-digit code's 5 minutes, since clicking a link in WhatsApp takes longer than typing a code that's already on screen. Same 60s resend cooldown and "only one live token per phone" invalidation as before.
- **Dev/QA login link** (`GET /api/patients/:id/dev-login-code`, staff roles only) — surfaces the current magic link in plaintext for the staff `/patients/[id]` page's eye-icon reveal, so QA doesn't have to tail server logs (or have a real WhatsApp number) to log in as a test patient. `OtpCodeEntity.devPlainCode` (nullable) is populated by `SmsAuthService#shouldExposeDevPlainCode` only when **both** `NODE_ENV !== 'production'` and `WHATSAPP_ACCESS_TOKEN`/`WHATSAPP_PHONE_NUMBER_ID` are unset — null (and thus invisible) the moment either stops being true, so a real deployment never has a plaintext link to leak.
